...
local core = getCore()
if not core.getVersionNumber and core.getVersion then
...
function MainOptions:create()
	...
	self.gameOptions.toUI = function(self)
		if ModOptions and type(ModOptions.loadFile) == "function" then ModOptions:loadFile() end
		if ModOptions and type(ModOptions.TrySave) == "function" then ModOptions:TrySave() end
	...
	if is_ingame then
		if ModOptions and type(ModOptions.loadFile) == "function" then
			ModOptions:loadFile()
		end
	...
	local function TryApplyOpt(data)
		...
		data.settings.options[data.id] = val
...
do
	...
	function SandboxOptionsScreen:settingsFromUI(options)
		if not ModOptions.was_SandboxOptions then
	...
	Events.OnGameTimeLoaded.Add(function()
		...
		local new_data = {}
		if ModOptions.was_SandboxOptions and gameTime:getWorldAgeHours() < 24 then
